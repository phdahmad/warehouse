...
let warehouseWidth, warehouseHeight, numItemsToPickGlobalDemand, numTotalAvailableItems, numWorkers, workerTripCapacity, pickingTimePerItem; // Added pickingTimePerItem
let allAvailableItemLocations = [];
let globalPickList = []; 
let depotLocation = { x: 0, y: 0, name: "Depot" };
let workersData = []; 
let simulationInProgress = false;
let selectedVrpAlgorithm = "round_robin_2opt"; 
...
function generateAllAvailableItemLocations() {
    ...
    for (let i = 0; i < numTotalAvailableItems; i++) {
        ...
        allAvailableItemLocations.push({ id: `avail-item-${i}`, x: x, y: y, name: `AvP${i}`, demand: 1 }); 
...
function assignItemsRoundRobin() {
    ...
    let allItemsAssignedInThisRound = true; 
        ...
        while (worker.currentLoad < workerTripCapacity && unassignedItems.length > 0) {
            ...
            if (itemToAssign) {
                if (worker.currentLoad + itemToAssign.demand <= workerTripCapacity) {
                    ...
                    unassignedItems.unshift(itemToAssign); 
                    break; 
                }
            } else {
                break;
...
function solveVrpWithClarkeWright() {
    ...
    let routes = globalPickList.map(item => ({
        nodes: [item], 
        load: item.demand, 
        cost: calculatePathCostForWorker([item], depotLocation, 0) // Pass 0 for pickingTime initially for C&W logic
    }));
    ...
    for (const saving of savingsList) {
        ...
        if (routeI && routeJ && routeIIndex !== routeJIndex) { 
            ...
            // Try merging J to the end of I's route
            if (iIsLastInRouteI && jIsFirstInRouteJ) {
                if (routeI.load + routeJ.load <= workerTripCapacity) {
                    ...
                    routes.splice(Math.max(routeIIndex, routeJIndex), 1); 
                    routes.splice(Math.min(routeIIndex, routeJIndex), 1);
                    routes.push({ nodes: mergedNodes, load: mergedLoad, cost: calculatePathCostForWorker(mergedNodes, depotLocation, 0) });
                    continue; // Move to next saving after a successful merge
                }
            }

            // Try merging I to the end of J's route (symmetric case)
            // This requires J to be last in its route and I to be first in its (or their routes to be reversed)
            // For this simplified C&W, we'll only implement the one merge direction above.
            // A full C&W would check more merge possibilities (e.g., merging internal nodes, reversing one route).
        }
    }
    ...
    if (routes.length > numWorkers) {
        ...
        // Sort routes by some criteria (e.g., number of items, or keep as is) before assigning to limited workers
        // routes.sort((a,b) => b.nodes.length - a.nodes.length); // Example: prioritize longer routes
    }

    for (let i = 0; i < Math.min(routes.length, numWorkers); i++) {
        workersData[i].itemsToPickThisTrip = routes[i].nodes; 
        workersData[i].path = routes[i].nodes; 
        workersData[i].currentLoad = routes[i].load;
        // Recalculate distance and time *with* pickingTimePerItem here
        workersData[i].distance = calculatePathCostForWorker(workersData[i].path, depotLocation, 0); // Distance only
        workersData[i].time = workersData[i].distance + (workersData[i].path.length * pickingTimePerItem);
    }
    ...
    if (numItemsToPickGlobalDemand > 0) {
        ...
        if (pickedItemIds.size < globalPickList.length) {
            allPickListItemsCovered = false;
             ...
             const warningMsg = (document.getElementById('vrpWarning').textContent ? document.getElementById('vrpWarning').textContent + "\n" : "") +
                               `تنبيه إضافي (Clarke & Wright): لم يتم تعيين ${unassignedCount} منتج من الطلب الكلي (${globalPickList.length}) 
...
// Modified to accept pickingTime for accurate cost during TSP optimization if needed,
// but for C&W savings calculation, pickingTime is often excluded, then added for final route time.
function calculatePathCostForWorker(path, workerStartPoint, currentPickingTimePerItem = 0) {
    ...
    // If currentPickingTimePerItem is passed (e.g. for final time calculation), add it.
    // For savings calculation in C&W, we might pass 0.
    const totalPickingTime = path.length * currentPickingTimePerItem;
    return totalDistance + totalPickingTime; // This now returns total *time* if pickingTime is included
}
...
function apply2OptForWorker(initialPath, workerStartPoint) {
    ...
    // For 2-Opt, we optimize based on distance first, then add picking time for the final time value.
    let bestDistance = calculatePathCostForWorker(bestPath, workerStartPoint, 0); // Distance only for 2-Opt comparisons
    ...
    while (improved && iterationCounter < maxIterations) {
        ...
        for (let i = 0; i < bestPath.length - 1; i++) {
            for (let k = i + 1; k < bestPath.length; k++) {
                ...
                const newDistance = calculatePathCostForWorker(newPath, workerStartPoint, 0); // Distance only
    ...
    return bestPath; // Returns the path optimized for distance
}

function solveVRPAndOptimizeRoutes() {
    if (selectedVrpAlgorithm === "clarke_wright_2opt") {
        solveVrpWithClarkeWright(); // This populates worker.itemsToPickThisTrip, worker.path (initially same as items), worker.distance, worker.time
        // C&W in this impl. already calculates final time including pickingTime.
        // We still apply 2-Opt to the *sequence* of items (path) for each worker.
        workersData.forEach(worker => {
            if (worker.itemsToPickThisTrip.length > 0) {
                // The path from C&W is already ordered. We optimize this order.
                const optimizedItemSequence = apply2OptForWorker(worker.itemsToPickThisTrip, depotLocation);
                worker.path = optimizedItemSequence; // Update path to the 2-Opt sequence
                // Recalculate distance and time based on the 2-Opt optimized path
                worker.distance = calculatePathCostForWorker(worker.path, depotLocation, 0); // Distance only
                worker.time = worker.distance + (worker.path.length * pickingTimePerItem);
            }
        });
...
async function startSimulation() {
    ...
    pickingTimePerItem = parseInt(document.getElementById('pickingTimePerItem').value); // Get picking time
    ...
    if (isNaN(warehouseWidth) || isNaN(warehouseHeight) || isNaN(numWorkers) || isNaN(workerTripCapacity) ||
        isNaN(pickingTimePerItem) || isNaN(numTotalAvailableItems) || isNaN(numItemsToPickGlobalDemand) ||
        warehouseWidth <= 1 || warehouseHeight <= 1 || numWorkers < 1 || workerTripCapacity < 1 || pickingTimePerItem < 0 ||
        numTotalAvailableItems < 0 || numItemsToPickGlobalDemand < 0) {
        displayMessage('vrpError', "الرجاء إدخال قيم صحيحة وموجبة لجميع حقول الإعدادات (السعة، العمال، وقت الجلب >= 0).");
        errorFound = true;
    }
    ...
    workersData.forEach(worker => {
        // worker.time already includes pickingTime from solveVRPAndOptimizeRoutes
        totalDistanceAllWorkers += worker.distance; // Distance is travel only
        ...
        detailP.innerHTML = `<b>عامل ${worker.id}:</b> ${worker.itemsToPickThisTrip.length} منتجات (الحمولة: ${worker.currentLoad}), 
                             ...
                             زمن: ${worker.time.toFixed(2)} (منها ${worker.itemsToPickThisTrip.length * pickingTimePerItem} وقت جلب)
    ...
    if (globalPickList.length > 0 || numItemsToPickGlobalDemand === 0) {
        ...
        document.getElementById('vrpMaxTimeText').textContent = `أقصى زمن لعامل واحد (يشمل وقت الجلب): ${maxTimeSingleWorker.toFixed(2)} وحدات زمن`;
        document.getElementById('vrpTotalTimeText').textContent = `مجموع أزمنة العمال (يشمل وقت الجلب): ${sumOfIndividualWorkerTimes.toFixed(2)} وحدات زمن`;
